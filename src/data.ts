...
export async function initiateVoiceflowCall({ phoneNumber, card, fraudAlert }: { phoneNumber: string; card: string; fraudAlert: 'yes' | 'no' }): Promise<boolean> {
	try {
		console.log(`Initiating call to phone number: ${phoneNumber}`);

		// Build the base URL for the call endpoint
		const url = `${getBaseUrl()}/api/voiceflow/call?phone=${encodeURIComponent(phoneNumber)}&card=${encodeURIComponent(card)}&fraudAlert=${encodeURIComponent(fraudAlert)}`;
