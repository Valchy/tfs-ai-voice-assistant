...
export async function initiateVoiceflowCall({ phoneNumber, card }: { phoneNumber: string; card: string }): Promise<boolean> {
	try {
		console.log(`Initiating call to phone number: ${phoneNumber}`);

		// Build the base URL for the call endpoint
		const url = `${getBaseUrl()}/api/voiceflow/call?phone=${encodeURIComponent(phoneNumber)}&card=${encodeURIComponent(card)}`;
